...
export class UsersService implements OnModuleInit {
  ...
  async verifyUser(email: string, password: string) {
    ...
    // await this.updateUserRememberMe(user.id, rememberMe);
    return user;
  }

  // 사용자 이메일로 사용자 데이터 가져오기
  ...
  // 사용자 비밀번호 검증 로직
  private async validateUserCredentials(user: User, password: string) {
  ...
  private validatePhoneNumber(
    ...
    if (!numberInfo || !PHONE_REGEX.test(numberInfo.nationalNumber)) {
  ...
  async updateUserRememberMe(userId: number, rememberMe: boolean) {
    return this.prismaService.user.update({
      where: { id: userId },
      data: { rememberMe: rememberMe ?? false },
    });
  }
  ...
  async decodeActivationToken(token: string) {
    try {
      return this.jwtService.verify<{ id: number; activationCode: string }>(
        token,
        {
          secret: this.configService.getOrThrow(
            'AUTH_JWT_ACTIVATION_TOKEN_SECRET',
  ...
  async resendVerificationCode(
    ...
    try {
      ...
      await this.notifyEmailVerification(
        ...
        '이메일 인증 코드 재전송',
        'activation-mail',
        // join(__dirname, '../../', '/email-templates/activation-mail')
      );
  ...
  private validateTokenExpiration(decodedToken: { exp?: number }) {
